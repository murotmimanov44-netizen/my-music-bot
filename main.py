...
from pytgcalls.types import MediaStream # Көпчүлүк жаңы версияларда ушул колдонулат
...
async def play_video(client, message):
    ...
    try:
        # Эң универсалдуу ойнотуу ыкмасы
        await call_py.play(
            message.chat.id,
            MediaStream(url)
        )
    except Exception as e:
        # Эгер play иштебесе, эски join ыкмасын байкап көрөт
        try:
